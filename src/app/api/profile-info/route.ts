...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Sichtbarkeits-Spalten evtl. noch nicht angelegt → ohne sie laden.
    row = (await sql`
      SELECT user_name, avatar, COALESCE(bio, '') AS bio, color,
             COALESCE(martial_arts, '[]'::jsonb) AS martial_arts,
             COALESCE(skills, '{}'::jsonb) AS skills
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { avatar, bio, color, martial_arts, skills, profile_visibility, profile_visibility_group } = await req.json();
    const sql = getSql();
    if (avatar !== undefined) {
